...
const hasUnsavedChanges = () => {
  ...
  return Object.keys(initial).some((key) => {
    const field = key as keyof ThirdSalesOrderPayload
  ...
  }) || evidenceFileList.value.length > 0 || paymentScreenshotList.value.length > 0 || chatRecordFileList.value.length > 0
}
...
<template>
  <el-drawer
    v-model="visible"
    :title="isEditMode() ? '编辑三销业绩' : '录入三销业绩'"
    size="700px"
